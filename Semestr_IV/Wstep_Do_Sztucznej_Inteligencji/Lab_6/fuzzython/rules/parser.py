...
def parse_rule(sentence, scope, tnorm, snorm, cnorm):
    ...
    from fuzzython.operators.operator import Operator
    ...
    if system_type == 's' or (not IS in then_): # Sugeno Consequent
        from fuzzython.rules.srule import SRule
        ...
        if system_type == 't':
            from fuzzython.rules.trule import TRule
            return TRule(antecedent, consequent[0], weight)
        from fuzzython.rules.mrule import MRule
