...
FROM
    GenreBorrowCount
WHERE
    BORROW_COUNT > (SELECT AVG_BORROW_COUNT FROM AverageBorrowCount);
SELECT
    G.G_NAZWA AS GENRE_NAME,
    COUNT(K.ID_KS) AS BOOK_COUNT,
    SUM(K.CENA) AS TOTAL_PRICE
FROM
    KSIAZKA K
JOIN
    GATUNEK G ON K.ID_GAT = G.ID_GAT
GROUP BY
    G.G_NAZWA
ORDER BY
    G.G_NAZWA;